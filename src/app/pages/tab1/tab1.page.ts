...
export class Tab1Page implements OnInit {
  ...
  addPageConfig(doc:jsPDF) {
    for (let i = 1; i <= doc.getNumberOfPages(); i++) {
      ...
      this.generarCabezera(doc);
      this.generarFooter(doc,i);

     
    }
  }
  generarCabezera(doc:jsPDF) {
    ...
    const imgX = 397; // Posición X de la imagen
    ...
    doc.text(texto, 20, 20, {baseline:'bottom'});
  }
  generarFooter(doc:jsPDF,i:number) {
    doc.setFillColor('#CCCCCC');
    doc.rect(10, doc.internal.pageSize.height - 20,774,10, 'F');
      
    // Añadimos la paginación
    doc.text("Página "+i + " de " + doc.getNumberOfPages(), doc.internal.pageSize.width - 397, doc.internal.pageSize.height - 10, {baseline:'bottom'});
